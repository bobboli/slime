...
CHECK_WEIGHT_UPDATE_SKIP_LIST="${CHECK_WEIGHT_UPDATE_SKIP_LIST:-visual.}"
...
if [[ "${CHECK_WEIGHT_UPDATE_EQUAL}" == 1 ]]; then
  ...
  if [[ -n "${CHECK_WEIGHT_UPDATE_SKIP_LIST}" ]]; then
    read -r -a CHECK_WEIGHT_UPDATE_SKIP_PATTERNS <<<"${CHECK_WEIGHT_UPDATE_SKIP_LIST}"
    MISC_ARGS+=(--check-weight-update-skip-list "${CHECK_WEIGHT_UPDATE_SKIP_PATTERNS[@]}")
  fi
fi
